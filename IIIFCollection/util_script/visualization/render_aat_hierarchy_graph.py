...
def generate_dot(dot_path: Path) -> None:
    subprocess.run(
        [
            'python',
            str(Path(__file__).resolve().parent / 'generate_aat_hierarchy_dot.py'),
        ],
        check=True,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Generate AAT hierarchy DOT and optional SVG/PNG output.'
    )
    parser.add_argument(
        '--format',
        choices=['svg', 'png'],
    ...
    parser.add_argument(
        '--dot',
        default='aat_hierarchy_graph.dot',
        help='DOT file path to generate or render.',
    )
    parser.add_argument(
        '--output',
        help='Rendered output file path. Defaults to aat_hierarchy_graph.<format>.',
    )
    ...
    output_path = (
        ...
        if args.output
        else script_dir / f'aat_hierarchy_graph.{args.format}'
    )
