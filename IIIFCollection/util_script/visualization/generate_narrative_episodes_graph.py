...
OUTPUT_DOT = SCRIPT_DIR / 'narrative_episodes_concept_graph.dot'

# Edit this list to choose the narrative episode concepts that should be included by default.
INPUT_CONCEPTS = [
    'mdhn:Story_of_Moses',
    'mdhn:Qarun_swallowed_by_the_earth',
    'mdhn:iconclass71V42',
    # Add more narrative episode concepts here.
]

SKOS_EXACT = 'skos:exactMatch'
SKOS_RELATED = 'skos:relatedMatch'
SKOS_RELATED_ALT = 'skos:relatedMath'
...
def parse_narrative_ontology(path: Path) -> Dict[str, Dict[str, Set[str]]]:
    if not path.exists():
        raise FileNotFoundError(f'Narrative episodes ontology not found: {path}')
    ...
    for raw_line in path.read_text(encoding='utf-8').splitlines():
        ...
        if SKOS_RELATED in line or SKOS_RELATED_ALT in line:
            values = re.findall(r'mdhn:[A-Za-z0-9_]+', line)
...
def collect_resource_terms(resource: Any, allowed_terms: Set[str]) -> Dict[str, Set[str]]:
...
def iter_page_like_resources(resource: Any, parent_id: str, parent_label: str, allowed_terms: Set[str]) -> List[Dict[str, Any]]:
    ...
    if isinstance(metadata, list):
        for meta in metadata:
            ...
            if not is_states_or_ascanvas_label(label):
                continue
            ...
            for entry in entries:
                ...
                entry_label = entry.get('label') or entry.get('folio') or parent_label
                if not entry_label:
                    continue

                tags = collect_resource_terms(entry, allowed_terms)
                entry_terms = tags['all']
                if not entry_terms:
                    entry_terms = collect_resource_terms({'depicts': entry.get('depicts')}, allowed_terms)['all']

                page_resources.append(
                    {
                        'id': entry.get('canvas') or entry.get('id') or f'{parent_id}:{len(page_resources)+1}',
                        'label': str(entry_label),
                        'collection': parent_id,
                        'collection_label': parent_label,
                        'concepts': sorted(entry_terms),
                        'direct_concepts': sorted(tags['direct']),
                        'canvas_concepts': sorted(tags['canvas']),
                        'page_folio': entry.get('folio'),
                        'page_canvas': entry.get('canvas'),
                        'source_resource': parent_label,
                    }
                )
...
def find_relating_resources(root: Path, concepts: Set[str], ontology_concepts: Set[str]) -> List[Dict[str, Any]]:
    ...
    for path in files:
        ...
        for resource in collection.get('manifests', []) + collection.get('items', []):
            ...
            page_resources = iter_page_like_resources(resource, resource_id, resource_label, ontology_concepts)
            if page_resources:
                for page_resource in page_resources:
                    tags = {
                        'direct': set(page_resource['direct_concepts']),
                        'canvas': set(page_resource['canvas_concepts']),
                        'all': set(page_resource['concepts']),
                    }
                    if not tags['all']:
                        continue
                    ...
                    resources.append(
                        {
                            'id': page_resource['id'],
                            'label': page_resource['label'],
                            'collection': str(path.name),
                            'collection_label': collection_label,
                            'concepts': sorted(tags['all']),
                            'direct_concepts': sorted(tags['direct']),
                            'canvas_concepts': sorted(tags['canvas']),
                            'page_folio': page_resource.get('page_folio'),
                            'page_canvas': page_resource.get('page_canvas'),
                            'source_resource': page_resource.get('source_resource'),
                        }
                    )
                continue

            tags = collect_resource_terms(resource, ontology_concepts)
            if not tags['all']:
                continue
            if not tags['all'].intersection(concepts):
                continue
...
def build_graph(
    ...
    dot_lines: List[str] = [
        'digraph NarrativeEpisodesGraph {',
        '  rankdir=LR;',
    ...
    concept_style: Dict[str, Tuple[str, str, str]] = {}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Generate a DOT graph for Narrative Episode concepts and matching resources.'
    )
    parser.add_argument(
        '--dot',
        type=Path,
    ...
    ontology = parse_narrative_ontology(ONTOLOGY_PATH)
